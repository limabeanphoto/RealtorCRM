...
import { FaPhone, FaEdit, FaEnvelope, FaBuilding, FaTasks, FaHistory, FaTrash, FaUserEdit } from 'react-icons/fa';
...
// Utility function to get color based on outcome
...
// Utility function to get assigned status badge style
...
export default function ContactCard({ 
  ...
  onContactUpdate,
  currentUser,
  onReassignClick // For admin reassignment
}) {
  ...
  // Fetch related data when expanding for the first time
  const fetchRelatedData = async (isExpanded) => {
  ...
  // Task status change from within the contact card
  ...
  // If contact data isn't available yet, don't render
  if (!contact) {
    return null;
  }
  ...
  return (
    <BaseCard
      ...
      onToggleExpand={fetchRelatedData}
    />
  );
}